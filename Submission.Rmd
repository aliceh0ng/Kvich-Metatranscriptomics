...
HUMANn3 functional profiling was applied to the KneadData-cleaned reads (84 samples; La98 and La125 excluded). This analysis, which was not performed in the original study, and I hoped to characterize community-wide and species-resolved metabolic activity across CRC, paired normal, and healthy tissue. HUMANn3 accepts a single-end input file and a taxonomic pre-screen in MetaPhlAn3 format, requiring two preparatory steps per sample: converting the Bracken report to MetaPhlAn3 format, and concatenating R1 + R2 into a single FASTQ.
...
An inital PCA showes:

Then MaSaalin2 was used to ....
...
res_cp   <- read.delim("maaslin_crc_vs_paired/all_results.tsv")
res_ch   <- read.delim("maaslin_crc_vs_healthy/all_results.tsv")
res_comb <- read.delim("maaslin_crc_vs_noncrc/all_results.tsv")
...
top_de %>% transmute(Pathway = feature %>%
      str_replace_all("\\.", " ") %>%
      str_trunc(67), Coefficient = round(coef, 3), `p-value` = signif(pval, 3), `q-value` = signif(qval, 3)) %>%
...
**Figure 9.** Paired slope plot of the top 9 CRC-associated pathways from MaAsLin2
...
- This re-analysis set out to determine whether standard low-biomass metatranscriptomic workflows could recover additional microbial insight from the Kvich et al. colorectal cancer biopsy dataset beyond the original two-reference genome approach.
- First, reproducing the published microbial analyses on the raw unmapped reads successfully recapitulated the key reported findings, including low but structured alignment to B. fragilis and F. nucleatum, enrichment of Fusobacterium and F. nucleatum in CRC samples, and increased expression of the virulence-associated genes fomA and radD in high-F. nucleatum libraries.
- The first part of this analysis shows that applying an additional round of KneadData preprocessing does not substantially alter these major community composition patterns, instead reducing low-depth outliers and tightening the same CRC-associated abundance trends. 
- Comparing the two Figure 4 recreations, the overall bacterial scaled counts became more consistent across sample groups after KneadData preprocessing, which is likely driven by removal of the lowest-depth outlier libraries. In contrast, the within-sample alpha diversity differences between groups became slightly more pronounced, although the CRC versus paired comparison remained non-significant. This is biologically reasonable, as matched CRC and paired normal samples originate from the same patient and are therefore expected to be more similar to one another than to unrelated healthy controls, where inter-patient variability contributes more strongly. Importantly, the key disease-associated trends were preserved after preprocessing: Fusobacterium counts remained enriched in CRC samples, and the same overall pattern was observed for relative abundance of both F. nucleatum and B. fragilis.
- By processing these reads through Kraken2/Bracken, bioBakery's 
