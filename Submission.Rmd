...
**Sample naming:** `Lasse4--Lasse33` in file names correspond to `La4--La33` in clinical metadata (same samples, different naming convention). `La125` is a sequencing control absent from the metadata and is excluded from all analyses. All R scripts remap names with `str_replace("^Lasse(\\d+)$", "La\\1")`.
...
![samtools-flagstat-pct-table](no_kneaddata/qc/multiqc/samtools-flagstat-pct-table.png)

- The report shows that mapping rate to the combined B. fragilis + F. nucleatum reference is very low, 
as expected by mapping all unmapped reads to just two species. However, some samples (La70 13.8%, La122 12.95, La110 10.64) showed a high percentage. These were also high depth (large number of reads) samples.
- Mean mapping rate is around 2.8%. From the violin plot look appears to be (similar), showing that most mapped pairs do pair properly
...
## 1.6 Taxonomic profiling -- Kraken2 (script: `04_kraken2.sh`)
...
## 1.7 Abundance estimation -- Bracken (script: `05_bracken.sh`)
...
Community composition results are shown for both the raw reads (119 samples, reproducing Kvich et al.) and KneadData-cleaned reads (86 samples) side-by-side. Gene expression results (Table S2, Figure 2J) are based on BWA alignment and featureCounts, which were applied to the raw reads only.
...
rename_lasse <- function(df) {
  colnames(df) <- colnames(df) %>% str_replace("^Lasse(\\d+)$", "La\\1")
  df
}
...
parse_flagstat <- function(file) {
  ...
  data.frame(sample = tools::file_path_sans_ext(basename(file)),
             total_fragments = as.numeric(str_extract(grep("read1", lines, value = TRUE)[1], "^\\d+")))}
...
species_df <- data.frame(
  ...
  left_join(meta, by = "sample") %>% filter(!is.na(type)) %>%
  mutate(bfrag_rel = `Bacteroides fragilis`    / total_per_sample[sample],
         fnuc_rel  = `Fusobacterium nucleatum` / total_per_sample[sample])
...
species_df_kd <- data.frame(
  ...
  left_join(meta, by = "sample") %>% filter(!is.na(type)) %>%
  mutate(bfrag_rel = `Bacteroides fragilis`    / total_per_sample_kd[sample],
         fnuc_rel  = `Fusobacterium nucleatum` / total_per_sample_kd[sample])
...
### Figure S6 -- Non-human read mapping per sample

```{r fig-s6, fig.width=14, fig.height=5}
fill_scale <- scale_fill_manual(
  name   = "Mapping",
  values = c("B. fragilis" = "#A0522D", "F. nucleatum" = "#1B6CA8", "Unmapped" = "#BEBEBE")
)

sort_by_total <- function(df) {
  order <- df %>% group_by(sample) %>%
...
p_s6a + p_s6b + plot_layout(guides = "collect") +
  plot_annotation(title = "Figure S6 -- Non-human read mapping per sample")
```

### Table S2 -- Top 20 expressed genes per organism (CRC samples, >=1M reads)

From Kvich et al. supplemental methods: reads normalized with DESeq2 variance-stabilizing transformation (VST); cutoff of 1M reads applied; expression averaged across passing samples; only protein-coding genes with a named product included.

```{r table-s2, results='asis'}
gene_annot <- read.delim("no_kneaddata/alignment/gene_annotations.tsv") %>%
  filter(product != "")
...
bfrag_avg <- run_vst(counts_raw %>% filter(Chr == BFRAG_CHR) %>% pull(Geneid))
...
table_s2 <- bind_rows(make_top20(bfrag_avg, "B. fragilis"),
                       make_top20(fnuc_avg,  "F. nucleatum"))
knitr::kable(table_s2, caption = "Table S2 -- Top 20 expressed genes per organism in CRC samples with >=1M reads.")
```

### Figure 2J - Virulence gene expression vs F. nucleatum abundance

fomA and radD are the two F. nucleatum virulence genes unambiguously annotated in GCF_016889925.1; featureCounts CDS counts are used in place of the authors' custom alignment.

```{r fig-2j, fig.width=8, fig.height=4}
...
ggplot(plot_df, aes(x = fnuc_scaled, y = scaled_count, colour = type)) +
  ...
  labs(x = "F. nucleatum abundance (Bracken scaled counts)",
       ...
       title = "Figure 2J -- fomA and radD vs F. nucleatum abundance") +
  theme_classic()
```

- These three plots match the results reported by Kvich et al. for the gene expression analysis.

### Figure 4 - Community composition by sample group

Two implementations are shown: the authors' original approach and a methodologically improved version. Key differences are summarised in the table below.

```{r fig4-pvals}
prep_paired <- function(df, value_col) {
  df %>%
    filter(type %in% c("CRC", "Paired"), patient_ID != 54) %>%
    group_by(patient_ID) %>% filter(n() == 2) %>% ungroup() %>%
    arrange(patient_ID, type) %>% select(patient_ID, type, all_of(value_col))
}

# Authors' p-values (one-sided, row-order pairing, Bonferroni across all comparisons per panel)
pA <- p.adjust(c(
  total_df %>% filter(type %in% c("CRC","Healthy")) %>%
    wilcox_test(total_scaled ~ type, alternative = "greater") %>% pull(p),
  prep_paired(total_df, "total_scaled") %>%
    wilcox_test(total_scaled ~ type, paired = TRUE, alternative = "greater") %>% pull(p)
))
pB <- p.adjust(c(
  shannon_df %>% filter(type %in% c("CRC","Healthy")) %>%
    t_test(shannon ~ type, alternative = "greater") %>% pull(p),
  shannon_df %>% filter(type %in% c("Paired","Healthy")) %>%
    t_test(shannon ~ type, alternative = "greater") %>% pull(p),
  prep_paired(shannon_df, "shannon") %>%
    t_test(shannon ~ type, paired = TRUE, alternative = "greater") %>% pull(p)
))
pC <- p.adjust(c(
  fuso_phylum_df %>% filter(type %in% c("CRC","Healthy")) %>%
    wilcox_test(fusobacteria ~ type) %>% pull(p),
  prep_paired(fuso_phylum_df, "fusobacteria") %>%
    wilcox_test(fusobacteria ~ type, paired = TRUE) %>% pull(p)
))
pD <- p.adjust(c(
  species_df %>% filter(type %in% c("CRC","Healthy")) %>%
    wilcox_test(fnuc_rel ~ type, alternative = "greater") %>% pull(p),
  prep_paired(species_df, "fnuc_rel") %>%
    wilcox_test(fnuc_rel ~ type, paired = TRUE, alternative = "greater") %>% pull(p),
  species_df %>% filter(type %in% c("CRC","Healthy")) %>%
    wilcox_test(bfrag_rel ~ type, alternative = "greater") %>% pull(p),
  prep_paired(species_df, "bfrag_rel") %>%
    wilcox_test(bfrag_rel ~ type, paired = TRUE, alternative = "greater") %>% pull(p)
))

# Improved p-values (two-sided, patient_ID join pairing, Bonferroni per panel)
paired_w2 <- function(df, col) {
  df %>% filter(type %in% c("CRC","Paired")) %>%
    select(patient_ID, type, all_of(col)) %>%
    pivot_wider(names_from = type, values_from = all_of(col)) %>% drop_na() %>%
    summarise(p = wilcox.test(CRC, Paired, paired = TRUE)$p.value) %>% pull(p)
}
paired_t2 <- function(df, col) {
  df %>% filter(type %in% c("CRC","Paired")) %>%
    select(patient_ID, type, all_of(col)) %>%
    pivot_wider(names_from = type, values_from = all_of(col)) %>% drop_na() %>%
    summarise(p = t.test(CRC, Paired, paired = TRUE)$p.value) %>% pull(p)
}
ind_w2 <- function(df, col) wilcox.test(df[[col]][df$type=="CRC"], df[[col]][df$type=="Healthy"])$p.value
ind_t2 <- function(df, col)    t.test(df[[col]][df$type=="CRC"], df[[col]][df$type=="Healthy"])$p.value

qA <- p.adjust(c(ind_w2(total_df, "total_scaled"),       paired_w2(total_df, "total_scaled")))
qB <- p.adjust(c(ind_t2(shannon_df, "shannon"),            paired_t2(shannon_df, "shannon")))
qC <- p.adjust(c(ind_w2(fuso_phylum_df, "fusobacteria"),   paired_w2(fuso_phylum_df, "fusobacteria")))
qD_b <- p.adjust(c(ind_w2(species_df, "bfrag_rel"),        paired_w2(species_df, "bfrag_rel")))
qD_f <- p.adjust(c(ind_w2(species_df, "fnuc_rel"),         paired_w2(species_df, "fnuc_rel")))

# KneadData p-values (improved methodology)
qA_kd   <- p.adjust(c(ind_w2(total_df_kd, "total_scaled"),      paired_w2(total_df_kd, "total_scaled")))
qB_kd   <- p.adjust(c(ind_t2(shannon_df_kd, "shannon"),          paired_t2(shannon_df_kd, "shannon")))
qC_kd   <- p.adjust(c(ind_w2(fuso_phylum_df_kd, "fusobacteria"), paired_w2(fuso_phylum_df_kd, "fusobacteria")))
qD_b_kd <- p.adjust(c(ind_w2(species_df_kd, "bfrag_rel"),        paired_w2(species_df_kd, "bfrag_rel")))
qD_f_kd <- p.adjust(c(ind_w2(species_df_kd, "fnuc_rel"),         paired_w2(species_df_kd, "fnuc_rel")))

# Convert to factor after all stat tests
total_df       <- to_factor(total_df);       total_df_kd       <- to_factor(total_df_kd)
shannon_df     <- to_factor(shannon_df);     shannon_df_kd     <- to_factor(shannon_df_kd)
fuso_phylum_df <- to_factor(fuso_phylum_df); fuso_phylum_df_kd <- to_factor(fuso_phylum_df_kd)
species_df     <- to_factor(species_df);     species_df_kd     <- to_factor(species_df_kd)
...
Reproduction of the authors' Figure 4 (one-sided tests, geom_sina, row-order pairing as in the original):

```{r fig4-authors, fig.width=12, fig.height=9}
pa_A <- ggplot(total_df, aes(x = type, y = total_scaled / 1e6)) +
  geom_boxplot(width = 0.4, color = "grey40", outlier.shape = NA) +
  geom_sina(color = "grey30", size = 1.5, maxwidth = 0.5, alpha = 0.8) +
  geom_signif(comparisons = list(c("CRC","Healthy"), c("CRC","Paired")),
              annotations = fmt_p(pA), step_increase = 0.12, tip_length = 0.02, textsize = 3) +
  ylim(0, max(total_df$total_scaled/1e6) * 1.3) +
  labs(x = NULL, y = "Scaled Counts (x10^6)",
       title = "A  Scaled counts assigned to\nBacteria superkingdom") + theme_bw()
pa_B <- ggplot(shannon_df, aes(x = type, y = shannon)) +
  geom_boxplot(outlier.shape = NA) + geom_jitter(position = position_jitter(0.1)) +
  geom_signif(comparisons = list(c("CRC","Healthy"), c("CRC","Paired"), c("Healthy","Paired")),
              annotations = fmt_p(pB), step_increase = 0.1, tip_length = 0.02, textsize = 3) +
  ylim(0, max(shannon_df$shannon) * 1.35) +
  labs(x = NULL, y = "Shannon Index", title = "B  Alpha Diversity") + theme_bw()
pa_C <- ggplot(fuso_phylum_df, aes(x = type, y = fusobacteria)) +
  geom_jitter(position = position_jitter(0.1)) +
  geom_signif(comparisons = list(c("CRC","Healthy"), c("CRC","Paired")),
              annotations = fmt_p(pC), step_increase = 0.12, tip_length = 0.02, textsize = 3) +
  ylim(0, max(fuso_phylum_df$fusobacteria) * 1.3) +
  labs(x = NULL, y = "Scaled Counts", title = "C  Fusobacteria Counts") + theme_bw()
pa_D <- ggplot(rel_long, aes(x = group, y = rel)) +
  geom_boxplot(outlier.shape = NA) + geom_jitter(width = 0.2, size = 0.8) +
  geom_signif(comparisons = list(c("B. fragilis\nCRC","B. fragilis\nHealthy"),
                                  c("F. nucleatum\nCRC","F. nucleatum\nHealthy")),
              annotations = fmt_p(pD[c(3,1)]),
              step_increase = 0.1, tip_length = 0.02, textsize = 3) +
  labs(x = NULL, y = "Relative abundance",
       title = "D  B. fragilis and F. nucleatum\nrelative abundance") +
  theme_bw() + theme(axis.text.x = element_text(size = 8))
(pa_A | pa_B) / (pa_C | pa_D) +
  plot_annotation(title = "Figure 4 -- Reproduction of Kvich et al. (raw reads, n=119)")
```

- I did not agree with the author's choice to do a one sided test (CRC > others) for panels A, B, and D. I chose to carry
forward with a two sided test throughout. Since most of the patients are paired (CRC vs paired), I chose to do paired testing.
Key methodological differences between the reproduction and improved implementation below:

Figure 4 repeated on KneadData-cleaned reads (86 samples) using the improved methodology:

```{r fig4-kd, fig.width=12, fig.height=9}
make_fig4_panels <- function(total, shannon, fuso, rel, qA, qB, qC, qD_b, qD_f, subtitle) {
  pA <- ggplot(total, aes(x = type, y = total_scaled / 1e6)) +
    geom_boxplot(outlier.shape = NA) + geom_jitter(width = 0.2, size = 0.8) +
    geom_signif(comparisons = list(c("CRC","Healthy"), c("CRC","Paired")),
                annotations = fmt_p(qA), step_increase = 0.12, tip_length = 0.02, textsize = 3) +
    ylim(0, max(total$total_scaled/1e6) * 1.3) +
    labs(x = NULL, y = "Scaled Counts (x10^6)",
         title = "A  Bacterial scaled counts", subtitle = subtitle) + theme_classic()
  pB <- ggplot(shannon, aes(x = type, y = shannon)) +
    geom_boxplot(outlier.shape = NA) + geom_jitter(width = 0.2, size = 0.8) +
    geom_signif(comparisons = list(c("CRC","Healthy"), c("CRC","Paired")),
                annotations = fmt_p(qB), step_increase = 0.1, tip_length = 0.02, textsize = 3) +
    ylim(0, max(shannon$shannon) * 1.3) +
    labs(x = NULL, y = "Shannon Index", title = "B  Alpha Diversity", subtitle = subtitle) + theme_classic()
  pC <- ggplot(fuso, aes(x = type, y = fusobacteria)) +
    geom_boxplot(outlier.shape = NA) + geom_jitter(width = 0.2, size = 0.8) +
    geom_signif(comparisons = list(c("CRC","Healthy"), c("CRC","Paired")),
                annotations = fmt_p(qC), step_increase = 0.12, tip_length = 0.02, textsize = 3) +
    ylim(0, max(fuso$fusobacteria) * 1.3) +
    labs(x = NULL, y = "Scaled Counts", title = "C  Fusobacteria Counts", subtitle = subtitle) + theme_classic()
  pD <- ggplot(rel, aes(x = group, y = rel)) +
    geom_boxplot(outlier.shape = NA) + geom_jitter(width = 0.2, size = 0.8) +
    geom_signif(comparisons = list(c("B. fragilis\nCRC","B. fragilis\nHealthy"),
                                    c("F. nucleatum\nCRC","F. nucleatum\nHealthy")),
                annotations = fmt_p(c(qD_b[1], qD_f[1])),
                step_increase = 0.1, tip_length = 0.02, textsize = 3) +
    labs(x = NULL, y = "Relative abundance",
         title = "D  B. fragilis / F. nucleatum relative abundance", subtitle = subtitle) +
    theme_classic() + theme(axis.text.x = element_text(size = 8))
  (pA | pB) / (pC | pD)
}

make_fig4_panels(total_df_kd, shannon_df_kd, fuso_phylum_df_kd, rel_long_kd,
                 qA_kd, qB_kd, qC_kd, qD_b_kd, qD_f_kd, "KneadData-cleaned (n=86)") +
  plot_annotation(title = "Figure 4 -- KneadData-cleaned reads")
```

### Figure S4 -- Phylum-level bacterial abundance

Phylum names from the 2026 Kraken2 database are harmonized to traditional names used in the paper (e.g. Bacillota → Firmicutes). Campylobacterota and related phyla are merged into Proteobacteria to match the older taxonomy used by the original authors. Species-level Bracken `counts_scaled` are summed by phylum using a taxonomy_id → phylum map built from the Kraken2 report tree (replicating the authors' taxize-based approach without API calls).

```{r s4-data, include=FALSE}
harmonize_phylum <- function(x) {
  case_when(
    x == "Bacillota"         ~ "Firmicutes",
    x == "Actinomycetota"    ~ "Actinobacteria",
    x == "Fusobacteriota"    ~ "Fusobacteria",
    x %in% c("Pseudomonadota","Campylobacterota",
              "Thermodesulfobacteriota","Thermodesulfobiota") ~ "Proteobacteria",
    x == "Synergistota"      ~ "Synergistetes",
    x == "Verrucomicrobiota" ~ "Verrucomicrobia",
    TRUE                     ~ x
  )
}

phylum_colors <- c(
  "Actinobacteria" = "#E6AB02", "Bacteroidota" = "#A6761D",
  "Firmicutes"     = "#66A61E", "Fusobacteria" = "#1B9E77",
  "Proteobacteria" = "#7570B3", "Synergistetes" = "#E7298A",
  "Verrucomicrobia"= "#D95F02", "Other"         = "#BEBEBE"
)

# Build taxonomy_id -> phylum map from Kraken2 report tree
build_tax_map <- function(report_dir) {
  list.files(report_dir, pattern = "\\.report$", full.names = TRUE) %>%
    map_dfr(function(f) {
      parsed <- strsplit(readLines(f), "\t")
      parsed <- parsed[sapply(parsed, length) == 6]
      if (!length(parsed)) return(NULL)
      ranks <- sapply(parsed, `[`, 4); taxids <- as.integer(sapply(parsed, `[`, 5))
      raw_names <- sapply(parsed, `[`, 6)
      depths <- nchar(raw_names) - nchar(trimws(raw_names, "left"))
      sk_vec <- character(length(ranks)); ph_vec <- character(length(ranks))
      cur_sk <- ""; cur_sk_d <- -1L; cur_ph <- ""; cur_ph_d <- -1L
      for (i in seq_along(ranks)) {
        d <- depths[i]; r <- ranks[i]
        if (cur_ph_d >= 0 && d <= cur_ph_d) { cur_ph <- ""; cur_ph_d <- -1L }
        if (cur_sk_d >= 0 && d <= cur_sk_d) { cur_sk <- ""; cur_sk_d <- -1L; cur_ph <- ""; cur_ph_d <- -1L }
        if (r == "D") { cur_sk <- trimws(raw_names[i]); cur_sk_d <- d }
        if (r == "P") { cur_ph <- trimws(raw_names[i]); cur_ph_d <- d }
        sk_vec[i] <- cur_sk; ph_vec[i] <- cur_ph
      }
      data.frame(taxonomy_id = taxids, superkingdom = sk_vec, phylum = ph_vec,
                 stringsAsFactors = FALSE) %>% filter(phylum != "")
    }) %>% distinct(taxonomy_id, .keep_all = TRUE)
}

build_phylum_df <- function(report_dir, bracken_dir, counts_mat, track_label) {
  tax_map <- build_tax_map(report_dir) %>% mutate(phylum = harmonize_phylum(phylum))
  name_phylum <- list.files(bracken_dir, pattern = "\\.bracken$", full.names = TRUE) %>%
    map_dfr(function(f) read.delim(f) %>% select(name, taxonomy_id)) %>%
    distinct(name, taxonomy_id) %>%
    left_join(tax_map, by = "taxonomy_id") %>%
    filter(superkingdom == "Bacteria", phylum != "") %>% distinct(name, phylum)
  counts_mat %>% rownames_to_column("name") %>%
    pivot_longer(-name, names_to = "sample", values_to = "scaled") %>%
    inner_join(name_phylum, by = "name") %>%
    group_by(sample, phylum) %>% summarise(scaled = sum(scaled), .groups = "drop") %>%
    left_join(meta %>% select(sample, type), by = "sample") %>%
    filter(!is.na(type)) %>% mutate(track = track_label)
}

phylum_df <- bind_rows(
  build_phylum_df("no_kneaddata/community_composition/kraken2",
                  "no_kneaddata/community_composition/bracken", counts_scaled,    "Raw reads"),
  build_phylum_df("kneaddata/community_composition/kraken2",
                  "kneaddata/community_composition/bracken",    counts_scaled_kd, "KneadData")
)

top7 <- phylum_df %>% group_by(phylum) %>% summarise(t = sum(scaled), .groups = "drop") %>%
  slice_max(t, n = 7) %>% arrange(desc(t)) %>% pull(phylum)

phylum_df <- phylum_df %>%
  mutate(phylum = ifelse(phylum %in% top7, phylum, "Other"),
         phylum = factor(phylum, levels = c(top7, "Other")),
         type   = factor(type, levels = type_levels),
         track  = factor(track, levels = c("Raw reads", "KneadData")))
```

```{r s4-counts, fig.width=12, fig.height=7}
phylum_df %>%
  filter(phylum != "Other") %>%
  ggplot(aes(x = type, y = scaled, colour = track)) +
  geom_boxplot(outlier.shape = NA, position = position_dodge(0.7), width = 0.5) +
  geom_jitter(position = position_jitterdodge(jitter.width = 0.15, dodge.width = 0.7),
              size = 0.7, alpha = 0.6) +
  facet_wrap(~phylum, scales = "free_y", nrow = 2) +
  scale_colour_manual(values = c("Raw reads" = "#555555", "KneadData" = "#1B9E77"),
                      name = "Track") +
  scale_y_continuous(labels = scales::comma) +
  labs(x = NULL, y = "Scaled counts",
       title = "Figure S4B -- Scaled counts for top phyla by sample group") +
...
Although the Kvich et al. unmapped reads had been adapter-trimmed and rRNA-depleted upstream, I ran the samples through another round of pre-processing to ensure no residual human sequences were carrying forward. I used the bioBakery's KneadData v0.12.4](https://huttenhower.sph.harvard.edu/kneaddata/), which combines Trimmomatic quality trimming with Bowtie2-based human read decontamination against T2T-CHM13v2.0 (hg39), and evaluate its effect on read quality, sample depth, and community composition relative to the Part 1 baseline.

Scripts: `scripts/kneaddata/01_kneaddata.sh` through `07_normalize_bracken.py`
...
Read counts vary widely across biopsy samples, which is consistent with the differences in microbial biomass seen throughout the dataset. Per-read quality scores were generally high across all samples.
...
Total read counts are reduced across samples after filtering, while read quality remains high. The large variation in retained depth between samples remains, which is expected for low-biomass biopsy-derived microbial reads.

## 2.4 Sample exclusions

33 samples were excluded due to insufficient read depth or high QC failure rates after KneadData:

- **Lasse samples:** Lasse4--Lasse33 except Lasse9, Lasse20, and Lasse25 (27 excluded)
- **La samples:** La50, La54, La68, La73, La77, La89 (6 excluded)


The 86 retained samples are listed in `samples_filtered.txt`.

## 2.5 Taxonomic profiling -- Kraken2 (script: `04_kraken2.sh`)

Same database and confidence threshold as Part 1. SLURM array: 86 tasks, 8 CPUs, 100 GB RAM, 24 h.
...
## 2.6 Abundance estimation -- Bracken (script: `05_bracken.sh`)
...
Community composition comparisons between raw and KneadData-cleaned reads are in section 1.9. This section covers read-level QC outcomes: read fate, depth distribution, excluded samples, and Kraken2 classification rates.

```{r kd-results}
# [R analysis -- to be added]
# - Plot read fate across KneadData stages per sample:
#     stacked bar of retained / removed by QC trim / removed by human decontamination
#     (from kneaddata/kneaddata_read_counts.tsv)
# - Plot post-KneadData read depth distribution with exclusion threshold marked
# - Table of excluded samples: sample ID, raw depth, KneadData depth, clinical group
# - Verify exclusions are not systematically biased by clinical group
# - Compare Kraken2 classification rates between raw and KneadData tracks
#     (for the 86 overlapping samples): % classified, % unclassified
# Note: community composition comparisons (Figure 4, Figure S4) are shown
# side-by-side with the raw reads results in section1.9 above.
```

# Part 3: Functional Profiling with HUMANn3

HUMANn3 functional profiling was applied to the KneadData-cleaned reads (84 samples; La98 and La125 excluded). This analysis, which was not performed in the original study, characterizes community-wide and species-resolved metabolic activity across CRC, paired normal, and healthy tissue.

HUMANn3 v3.6 was used (not v4, which was still in alpha at the time of analysis). HUMANn3 accepts a single-end input file and a taxonomic pre-screen in MetaPhlAn3 format, requiring two preparatory steps per sample: converting the Bracken report to MetaPhlAn3 format, and concatenating R1 + R2 into a single FASTQ.

**Note:** La98 and La125 were excluded: both are the highest read-depth outliers (La98: ~15M reads, La125: ~12.2M reads vs. median ~0.64M) and did not finish within the 24 h wall time.

## 3.1 Convert Bracken report to MetaPhlAn3 format (script: `06_bracken_to_mpa.py`)

HUMANn3's `--taxonomic-profile` argument accepts a MetaPhlAn3-format profile to bypass its internal MetaPhlAn step. The KrakenTools `kreport2mpa.py` script was not used due to compatibility issues; a custom script was written. It reads the Bracken-corrected `_bracken.report` and emits only the 7 standard NCBI ranks (Domain to Species) using MetaPhlAn3 prefix notation (`k__|p__|c__|o__|f__|g__|s__`). Species absent from ChocoPhlAn fall back to the UniRef90 translated search; they still contribute to pathway/gene-family tables but without nucleotide-level hits.

This step runs within the HUMANn3 SLURM script but can also be run independently:
...
Three steps per sample: (1) convert Bracken report to MetaPhlAn3 format (section3.1); (2) concatenate R1 + R2 -- HUMANn3 treats reads independently so concatenation loses no information, and the concatenated file is named `{sample}_cat.fq`, hence all HUMANn3 outputs contain `_cat_`; (3) run HUMANn3.
...
SLURM: 16 CPUs, 64 GB RAM, 24 h per task; `--array=0-85` (86 samples; La98 and La125 excluded).

Output per sample: `{sample}_cat_genefamilies.tsv` (UniRef90 gene families, RPK), `{sample}_cat_pathabundance.tsv` (MetaCyc pathway abundances, RPK), `{sample}_cat_pathcoverage.tsv` (MetaCyc pathway coverage, 0--1).

## 3.3 HUMANn3 post-processing

Once all samples were complete, per-sample tables are merged, normalized to copies per million (CPM), and split into stratified and unstratified components.
...
meta_h3    <- meta %>% filter(sample %in% h3_samples) %>%
...
```{r humann3-pcoa, fig.width=7, fig.height=5}
...
data.frame(sample = rownames(pcoa$points),PC1 = pcoa$points[,1],PC2 = pcoa$points[,2]) %>%
  ...
  theme_classic()
```
- The arc shape shows that the primary axis of functional variation follows a continuous gradient rather than discrete disease-associated clustering (between CRC and healthy or paired), reflecting the differences in microbial biomass we already observed and pathway detection sensitivity across low-biomass biopsy samples.

```{r humann3-diff-pathways, fig.width=12, fig.height=10, results='asis'}
...
top_paths <- kw_res %>% slice_head(n = 10) %>% pull(pathway)
...
path_long %>%
  ...
  labs(x = NULL, y = "Pathway abundance (CPM)",
       title = "Figure -- Top 15 pathways by Kruskal-Wallis (BH-adjusted)") +
  theme_classic() +
  theme(legend.position = "none", strip.text = element_text(size = 7),
        axis.text.x = element_text(size = 8))
```
- No statistically significantly different pathways. however, good to visualize still 

```{r humann3-species-contrib, fig.width=12, fig.height=8}
...
- hard to get statstically significant data from these kind of samples
- want to be robust by looking at everything (not just picking a couple like the authors did)
- similar to the authors, i've also run SAMSA2, a microbial analysis pipeline that uses DIAMOND, against just the b.fragilis genome, which seems to give insight, but with the high mapping %
...
References are hyperlinked throughout the document. Below is the full list in order of appearance.
...
3. Kvich L et al. (2024). [Full citation to be added]
