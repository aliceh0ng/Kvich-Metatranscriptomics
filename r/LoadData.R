source("/Users/alicehong/bmeg524/r/LoadLibs.R")
source("/Users/alicehong/bmeg524/r/HelperFunctions.R")
...
# Metadata ───────────────────────────────────────────────────────────────────
meta <- read.delim("/Users/alicehong/bmeg524/meta.tsv") %>%
...
multiqc <- read.delim("/Users/alicehong/bmeg524/kneaddata/qc/multiqc_pre/multiqc_report_data/multiqc_general_stats.txt") %>%
...
qual <- read.delim("/Users/alicehong/bmeg524/kneaddata/qc/multiqc_pre/multiqc_report_data/fastqc_per_base_sequence_quality_plot.txt",
                   check.names = FALSE) %>%
...
# featureCounts ──────────────────────────────────────────────────────────────
counts_raw    <- load_featurecounts("/Users/alicehong/bmeg524/no_kneaddata/alignment/counts/counts.txt")
kd_counts_raw <- load_featurecounts("/Users/alicehong/bmeg524/kneaddata/alignment/counts/counts.txt")
sample_cols   <- setdiff(colnames(counts_raw), c("Geneid", "Chr"))
gene_annot    <- read.delim("/Users/alicehong/bmeg524/no_kneaddata/alignment/gene_annotations.tsv") %>%
  filter(product != "")

# Flagstat ───────────────────────────────────────────────────────────────────
# Per-sample .flagstat files → total read pairs per sample (used in bfrag_fnuc)
flagstat_df <- list.files("/Users/alicehong/bmeg524/no_kneaddata/qc/flagstat",
                          pattern = "\\.flagstat$", full.names = TRUE) %>%
  map_dfr(parse_flagstat)

kd_flagstat_df <- read.delim("/Users/alicehong/bmeg524/kneaddata/qc/multiqc_bwa/bwa_alignment_qc_data/multiqc_samtools_flagstat.txt") %>%
  transmute(sample = clean_sample_ids(Sample), total_fragments = read1_passed)

# Full multiqc flagstat tables (used in Flagstat.R)
flagstat_kd   <- read.delim("/Users/alicehong/bmeg524/kneaddata/qc/multiqc_bwa/bwa_alignment_qc_data/multiqc_samtools_flagstat.txt") %>%
  mutate(pipeline = "KneadData")
flagstat_nokd <- read.delim("/Users/alicehong/bmeg524/no_kneaddata/qc/multiqc/bwa_alignment_qc_data/multiqc_samtools_flagstat.txt") %>%
...
kd <- read.delim("/Users/alicehong/bfrag3/output/new/qc/KneadDataQC/kneaddata_read_counts.tsv",
                 check.names = FALSE) %>%
...
kraken_files <- list.files("/Users/alicehong/bmeg524/kneaddata/community_composition/kraken2",
...
bracken_files <- list.files("/Users/alicehong/bmeg524/kneaddata/community_composition/bracken",
